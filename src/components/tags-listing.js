...
class GalleryListing extends React.Component {
  ...
  render( ) {
    ...
    const { currentPage, numPages, tag, allTags } = this.props.pageContext
        ...
        {Array.from({ length: numPages }, (_, i) => (
            ...
            >
              <Link className='page-numbers'
                to={`gallery/tags/${kebabCase(tag)}/${i === 0 ? '' : i + 1}`}
                style={{
              ...
              >
                {i + 1}
                      ...
                      {!isFirst && (
                      <PrevPage to={`gallery/tags/${kebabCase(tag)}/${prevPage}`} rel="prev">
                        ← Previous Page
                      </PrevPage>
                    )}
                    {!isLast && (
                      <NextPage to={`gallery/tags/${kebabCase(tag)}/${nextPage}`} rel="next">
                        Next Page →
                      </NextPage>
                    )}
                    </NavContainer>
                    ...
                    {allTags.map(tag => (
                      <button key={tag}>
                        <Link className="listingList" to={`/gallery/tags/${kebabCase(tag)}`}>{tag}</Link>
                      </button>
